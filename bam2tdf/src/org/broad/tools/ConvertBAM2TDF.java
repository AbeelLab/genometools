...
public class ConvertBAM2TDF {
		...
		for (String s : args) {
			if (!new File(s + ".bai").exists()) {
				...
				try {
					...
					e.printStackTrace();
					System.err.println("ERROR: Failed to create TDF file for " + s);
				}
			}
		}
	}
	...
	private static void createFile(String ifile) throws IOException, URISyntaxException {
		...
		Preprocessor p = new Preprocessor(new File(ifile).getName(), new File(ofile), dict, wfs);
